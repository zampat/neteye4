...
if [ ! -f "${DST_ITOA_AGENTS_WIN_FOLDER}/" ]
then
   mkdir -p ${DST_ITOA_AGENTS_WIN_FOLDER}
   echo "[i] Installing ITOA Agent for Windows of Version ${TELEGRAF_AGENT_VERSION} to ${DST_ITOA_AGENTS_WIN_FOLDER}"
   ...
   echo "[ ] ITOA Telegraf agent already installed"
fi
