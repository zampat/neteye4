...
function Read-SQLTraceConfiguratinFile
{
    ...
    if ($SQLTraceConfig)
    {
        ...
        [string] $AXBusinessDBname         = $null
        [string] $AXBusinessDS         = $null
        if ($SQLTraceConfig.'dmv.DX.AX2012' -ne $null)
        {
            $AXBusinessDBname = $SQLTraceConfig.'dmv.DX.AX2012'.databaseName #'AX6_CFA_DEV_Build'
            $AXBusinessDS     = $SQLTraceConfig.'dmv.DX.AX2012'.dataSource   
        }
        if ($SQLTraceConfig.'dmv.DX.D365FO' -ne $null)
        {
            $AXBusinessDBname = $SQLTraceConfig.'dmv.DX.D365FO'.databaseName #'AX6_CFA_DEV_Build'
            $AXBusinessDS     = $SQLTraceConfig.'dmv.DX.D365FO'.dataSource   
        }
...
function Validate-Permissions
{
    ...
    {
        $sqllogin   = $SQLAccount
        $sqlcmd     = "use master EXECUTE AS LOGIN ='$sqllogin' $perm  GO REVERT"
        [int] $result=$null
