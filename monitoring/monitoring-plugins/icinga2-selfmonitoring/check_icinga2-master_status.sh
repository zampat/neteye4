...
ICINGA_STATUS=`systemctl status icinga2-master >/dev/null 2>&1; echo $?`
ICINGA_STATUS_RUNNING=`systemctl status icinga2-master | head -n6 | grep "Active:" | sed 's/^[ \t]*//' | awk -F' since ' '{print $1}'`
