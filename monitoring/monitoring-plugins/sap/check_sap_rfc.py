...
def load_csv_configuration(filename : str, header : bool = True ) -> Dict[str, Dict[str, str]]:
    with open(filename, "r") as fd:
        ...
        for row in csv_reader:
            key = row[0]
            if key in conf:
                eprint("Error: multiple config for same object")
            else:
                conf[key] = {}
...
def parse_args():
    ...
    parser.add_argument('--function', '-f', dest='function_name', required=True, type=str, help="function, e.g. /WRP/NEMO_CENTRAL\n")
    parser.add_argument('--kpi', '-k', dest='key_performance_indicator', required=True, type=str, help="key performance indicator, e.g., WPSTA\n")
    ...
    parser.add_argument('--master-attribute', '-m', dest='master_attribute', required=False, type=str, help='master-attribute')
    # Finished
    parser.add_argument('--timeout', '-t', dest='timeout', required=False, default=25, type=int, help='timeout for the call (default 25)')
...
def main():
    ...
    if args.sysid not in sap_conf:
        print(f"No connection parameters available for system-ID {sysid:s}\n")
        exit(3)
